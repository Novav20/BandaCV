...
class ApplicationController:
    ...
    def _read_serial_data_loop(self):
        """Thread target for reading serial data continuously."""
    ...
    def stop(self):
        """Stops the application components and releases resources."""
        self.stop_event.set()
    ...
    def set_pwm(self, value: int):
        """Sets the PWM value and sends it to Arduino."""
        self.pwm_value = value
        self.serial_manager.send_command(self.pwm_value, self.current_servo_code)

    def set_active_classifier(self, classifier_key: str):
    ...
    def calibrate_camera(self):
        """Initiates camera calibration for size detection."""
    ...
    def process_video_frame(self):
        """Reads a frame, processes it, and updates UI."""
        frame = self.camera.read_frame()
